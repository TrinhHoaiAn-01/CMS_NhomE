/* global CMS_NhomEBgColors, CMS_NhomEPreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( CMS_NhomEBgColors ).forEach( function( context ) {
					CMS_NhomEGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function CMS_NhomEGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'CMS_NhomE-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#CMS_NhomE-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( CMS_NhomEPreviewEls[ context ] ) {
				_.each( CMS_NhomEPreviewEls[ context ], function( items, setting ) {
	...
	$( function() {
		CMS_NhomEGenerateColorA11yPreviewStyles( 'content' );
		CMS_NhomEGenerateColorA11yPreviewStyles( 'header-footer' );
	} );
