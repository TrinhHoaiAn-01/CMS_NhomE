/* global CMS_NhomEBgColors, CMS_NhomEColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( CMS_NhomEBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( CMS_NhomEBgColors[ context ].color ) {
						backgroundColorValue = CMS_NhomEBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( CMS_NhomEBgColors[ context ].setting ).get();
					}
					CMS_NhomESetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( CMS_NhomEBgColors ).forEach( function( context ) {
			wp.customize( CMS_NhomEBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					CMS_NhomESetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		CMS_NhomESetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				CMS_NhomESetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function CMS_NhomESetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = CMS_NhomEColor( backgroundColor, accentHue );
	 ...
	 * @return {void}
	 */
	function CMS_NhomESetRetineLogoVisibility( visible ) {
