...
if ( ! class_exists( 'CMS_NhomE_Walker_Page' ) ) {
	/**
	 * CUSTOM PAGE WALKER
	 ...
	 * @since CMS_NhomE 1.0
	 */
	class CMS_NhomE_Walker_Page extends Walker_Page {

		/**
		 * Outputs the beginning of the current element in the tree.
		 *
		 * @since CMS_NhomE 1.0
		 * @since CMS_NhomE 1.9 Renamed `$page` to `$data_object` and `$current_page` to `$current_object_id`
		...
		public function start_el( &$output, $data_object, $depth = 0, $args = array(), $current_object_id = 0 ) {
			...
			if ( '' === $page->post_title ) {
				...
				$page->post_title = sprintf( __( '#%d (no title)', 'CMS_NhomE' ), $page->ID );
			}
