 ...
 */
function CMS_NhomE_site_logo( $args = array(), $display = true ) {
	...
	/**
	 * Filters the arguments for `CMS_NhomE_site_logo()`.
	 *
	 * @since CMS_NhomE 1.0
	 *
	 ...
	 */
	$args = apply_filters( 'CMS_NhomE_site_logo_args', $args, $defaults );
	 ...
	 */
	$html = apply_filters( 'CMS_NhomE_site_logo', $html, $args, $classname, $contents );
 ...
 */
function CMS_NhomE_site_description( $display = true ) {
	 ...
	 */
	$html = apply_filters( 'CMS_NhomE_site_description', $html, $description, $wrapper );
 ...
 */
function CMS_NhomE_is_comment_by_post_author( $comment = null ) {
 ...
 */
function CMS_NhomE_filter_comment_reply_link( $link ) {

	$link = str_replace( 'class=\'', 'class=\'do-not-scroll ', $link );
	return $link;
}

add_filter( 'comment_reply_link', 'CMS_NhomE_filter_comment_reply_link' );
 ...
 */
function CMS_NhomE_the_post_meta( $post_id = null, $location = 'single-top' ) {

	echo CMS_NhomE_get_post_meta( $post_id, $location );
}
 ...
 */
function CMS_NhomE_edit_post_link( $link, $post_id, $text ) {
	if ( is_admin() ) {
		return $link;
	}
	...
	$text = sprintf(
		wp_kses(
			...
			__( 'Edit <span class="screen-reader-text">%s</span>', 'CMS_NhomE' ),
			array(
				'span' => array(
					'class' => array(),
				),
	...
	return '<div class="post-meta-wrapper post-meta-edit-link-wrapper"><ul class="post-meta"><li class="post-edit meta-wrapper"><span class="meta-icon">' . CMS_NhomE_get_theme_svg( 'edit' ) . '</span><span class="meta-text"><a href="' . esc_url( $edit_url ) . '">' . $text . '</a></span></li></ul><!-- .post-meta --></div><!-- .post-meta-wrapper -->';
}

add_filter( 'edit_post_link', 'CMS_NhomE_edit_post_link', 10, 3 );
 ...
 */
function CMS_NhomE_get_post_meta( $post_id = null, $location = 'single-top' ) {
	 ...
	 */
	$disallowed_post_types = apply_filters( 'CMS_NhomE_disallowed_post_types_for_meta_output', array( 'page' ) );
	...
	if ( 'single-top' === $location ) {
		...
		$post_meta = apply_filters(
			'CMS_NhomE_post_meta_location_single_top',
			array(
				'author',
				'post-date',
				'comments',
		 ...
		 */
		$post_meta = apply_filters(
			'CMS_NhomE_post_meta_location_single_bottom',
			array(
				'tags',
			)
		);
	...
	if ( $post_meta && ! in_array( 'empty', $post_meta, true ) ) {
		...
		<div class="post-meta-wrapper<?php echo esc_attr( $post_meta_wrapper_classes ); ?>">
			...
			<ul class="post-meta<?php echo esc_attr( $post_meta_classes ); ?>">
				 ...
				 *
				 * @since CMS_NhomE 1.0
				 * @since CMS_NhomE 1.1 Added the `$post_meta` and `$location` parameters.
				 *
				 * @param int    $post_id   Post ID.
				 ...
				 */
				do_action( 'CMS_NhomE_start_of_post_meta_list', $post_id, $post_meta, $location );
					...
					<li class="post-author meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text">
								...
								_e( 'Post author', 'CMS_NhomE' );
								?>
							</span>
							<?php CMS_NhomE_the_theme_svg( 'user' ); ?>
						</span>
					...
					<li class="post-date meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text">
								<?php
								/* translators: Hidden accessibility text. */
								_e( 'Post date', 'CMS_NhomE' );
								?>
							</span>
							<?php CMS_NhomE_the_theme_svg( 'calendar' ); ?>
						</span>
						<span class="meta-text">
					...
					<li class="post-categories meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text">
								<?php
								/* translators: Hidden accessibility text. */
								_e( 'Categories', 'CMS_NhomE' );
								?>
							</span>
							<?php CMS_NhomE_the_theme_svg( 'folder' ); ?>
						</span>
						<span class="meta-text">
							<?php _ex( 'In', 'A string that is output before one or more categories', 'CMS_NhomE' ); ?> <?php the_category( ', ' ); ?>
						</span>
					</li>
					<?php
				...
				if ( in_array( 'tags', $post_meta, true ) && has_tag() ) {
					...
					<li class="post-tags meta-wrapper">
						<span class="meta-icon">
							...
							<?php CMS_NhomE_the_theme_svg( 'tag' ); ?>
						</span>
						<span class="meta-text">
							<?php the_tags( '', ', ', '' ); ?>
						</span>
				...
				if ( in_array( 'comments', $post_meta, true ) && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
					...
					<li class="post-comment-link meta-wrapper">
						<span class="meta-icon">
							<?php CMS_NhomE_the_theme_svg( 'comment' ); ?>
						</span>
						<span class="meta-text">
				...
				if ( in_array( 'sticky', $post_meta, true ) && is_sticky() ) {
					...
					<li class="post-sticky meta-wrapper">
						<span class="meta-icon">
							<?php CMS_NhomE_the_theme_svg( 'bookmark' ); ?>
						</span>
						<span class="meta-text">
							<?php _e( 'Sticky post', 'CMS_NhomE' ); ?>
						</span>
					</li>
					<?php
				 ...
				 *                          Accepts 'single-top' or 'single-bottom'.
				 */
				do_action( 'CMS_NhomE_end_of_post_meta_list', $post_id, $post_meta, $location );
 ...
 */
function CMS_NhomE_filter_wp_list_pages_item_classes( $css_class, $page, $depth, $args ) {
...
add_filter( 'page_css_class', 'CMS_NhomE_filter_wp_list_pages_item_classes', 10, 4 );
 ...
 */
function CMS_NhomE_add_sub_toggles_to_main_menu( $args, $item ) {
	...
	if ( isset( $args->show_toggles ) && $args->show_toggles ) {
		...
		if ( in_array( 'menu-item-has-children', $item->classes, true ) ) {
			...
			$toggle_duration      = CMS_NhomE_toggle_duration();
			...
			$args->after .= '<button class="toggle sub-menu-toggle fill-children-current-color" data-toggle-target="' . $toggle_target_string . '" data-toggle-type="slidetoggle" data-toggle-duration="' . absint( $toggle_duration ) . '" aria-expanded="false"><span class="screen-reader-text">' .
				/* translators: Hidden accessibility text. */
				__( 'Show sub menu', 'CMS_NhomE' ) .
			'</span>' . CMS_NhomE_get_theme_svg( 'chevron-down' ) . '</button>';
...
add_filter( 'nav_menu_item_args', 'CMS_NhomE_add_sub_toggles_to_main_menu', 10, 2 );
 ...
 */
function CMS_NhomE_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	// Change SVG icon inside social links menu if there is supported URL.
	if ( 'social' === $args->theme_location ) {
		$svg = CMS_NhomE_SVG_Icons::get_social_link_svg( $item->url );
		if ( empty( $svg ) ) {
			$svg = CMS_NhomE_get_theme_svg( 'link' );
		}
...
add_filter( 'walker_nav_menu_start_el', 'CMS_NhomE_nav_menu_social_icons', 10, 4 );
 ...
 * @since CMS_NhomE 1.0
 */
function CMS_NhomE_no_js_class() {
...
add_action( 'wp_head', 'CMS_NhomE_no_js_class' );
 ...
 */
function CMS_NhomE_body_classes( $classes ) {
...
add_filter( 'body_class', 'CMS_NhomE_body_classes' );
 ...
 */
function CMS_NhomE_get_the_archive_title( $title ) {
	 ...
	 */
	$regex = apply_filters(
		'CMS_NhomE_get_the_archive_title_regex',
		array(
			'pattern'     => '/(\A[^\:]+\:)/',
...
add_filter( 'get_the_archive_title', 'CMS_NhomE_get_the_archive_title' );
 ...
 */
function CMS_NhomE_toggle_duration() {
	/**
	 ...
	 */
	$duration = apply_filters( 'CMS_NhomE_toggle_duration', 250 );
 ...
 */
function CMS_NhomE_unique_id( $prefix = '' ) {
	static $id_counter = 0;
